...
------------------------------------------------------------------------------*/
    ...
    {&out} '<script language="JavaScript" src="/scripts/js/tablesel.js"></script>'.

    {&out} htmlib-StartForm("mainform","post", appurl + '/demo/cust.p' ) skip.
    ...
    {&out}
        '<div id="toolbar">' skip
            htmlib-MntLink("add",?,appurl + '/' + "cust/custmnt.p",lc-link-otherp)
            '<span id="tboption"></span>'
        '</div>' skip.
    ...
    repeat while avail b-query:
        ...
        assign
            lc-rowobj = "ROW" + string(rowid(b-query))
            lc-toolobj = "TOOL" + string(rowid(b-query)).
        
        {&out}
            skip(2)
            '<tr class="tabrow1" id="' lc-rowobj '" onClick="javascript:rowSelect(this,~'' lc-toolobj '~')"'
                skip
                ' onmouseover="javascript:rowOver(this)"' skip
                ' onmouseout="javascript:rowOut(this)"' skip
           ...
           /* '<td valign="top" align="left">' */
            '<div id="' lc-toolobj '" style="display: none;">' skip
    ...
    do:
        ...
        if ll-prev 
        then {&out} htmlib-MntButton(appurl + '/' + "demo/cust.p","PrevPage","Prev Page").

      
        if ll-next 
        then {&out} htmlib-MntButton(appurl + '/' + "demo/cust.p","NextPage","Next Page").

    end.

    {&out} '</td><td align="right">' htmlib-ErrorMessage(lc-smessage) '&nbsp;' htmlib-SideLabel("Search").
    {&out} htmlib-InputField("search",20,lc-search)
          '&nbsp;' htmlib-MntButton(appurl + '/' + "demo/cust.p","search","Search")
          '&nbsp;' skip

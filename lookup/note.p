...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-StartForm("mainform","post", appurl + '/lookup/note.p' ).

    {&out} htmlib-Hidden("fieldname",lc-field1) skip
           htmlib-Hidden("description",lc-field2) skip.

    {&out} htmlib-ProgramTitle("Lookup Note").
   ...
   repeat while avail b-query:
       ...
       {&out}
           ...
           htmlib-AnswerHyperLink(lc-field1,
                             ...
                             b-query.notecode)
           '</td>'
           htmlib-TableField(html-encode(b-query.description),'left')
           '</tr>' skip.
   ...
   do:
       ...
       if ll-prev 
       then {&out} htmlib-LookupAction(appurl + '/' + "lookup/note.p","PrevPage","Prev Page").


       if ll-next 
       then {&out} htmlib-LookupAction(appurl + '/' + "lookup/note.p","NextPage","Next Page").

   end.

   {&out} '</td><td align="right">' htmlib-ErrorMessage(lc-smessage) '&nbsp;' htmlib-SideLabel("Search").
   {&out} htmlib-InputField("search",20,lc-search)
         '&nbsp;' htmlib-LookUpAction(appurl + '/' + "lookup/note.p","search","Search")
         '&nbsp;' skip
                 htmlib-HelpButton(appurl , "lookup/note" )
                 skip
         '</td></tr>'.

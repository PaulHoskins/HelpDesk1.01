...
def buffer b-query for webnote.
def buffer b-search for webnote.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-Header("Maintain Note Types") skip.

    {&out} htmlib-JScript-Maintenance() skip.

    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webnote.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Note Types") skip.
    

    {&out}
        tbar-StandardBar(
            appurl + "/sys/webnote.p",
            appurl + "/sys/webnotemnt.p",
            lc-link-otherp
            ).
    ...
    {&out}
            htmlib-TableHeading(
            "Note Type|Description|Customer View?"
            ) skip.
    ...
    do:
        find first b-search 
             where b-search.CompanyCode = lc-global-company
               and b-search.notecode >= lc-search no-lock no-error.
    ...
    repeat while avail b-query:
        ...
        {&out}
            ...
            htmlib-MntTableField(html-encode(b-query.notecode),'left')
            htmlib-MntTableField(html-encode(b-query.description),'left')
            htmlib-MntTableField(html-encode((if b-query.customercanview = true

...
def buffer b-query for company.
def buffer b-search for company.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-Header("Maintain Company") skip.

    {&out} htmlib-JScript-Maintenance() skip.

    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webcomp.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Company") skip.
    

    {&out}
        tbar-StandardBar(
            appurl + "/sys/webcomp.p",
            appurl + "/sys/webcompmnt.p",
            lc-link-otherp
            ).
    ...
    do:
        find first b-search 
             where b-search.CompanyCode >= lc-search no-lock no-error.
    ...
    repeat while avail b-query:
        ...
        {&out}
            ...
            skip
            htmlib-MntTableField(html-encode(b-query.CompanyCode),'left')

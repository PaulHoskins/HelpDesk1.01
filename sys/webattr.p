...
/***********************************************************************
    ...
    Program:        sys/webattr.p
    
    Purpose:        Web Attr Maintenance         
...
def buffer b-query for webattr.
def buffer b-search for webattr.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-Header("Maintain Web Attributes") skip.

    {&out} htmlib-JScript-Maintenance() skip.

    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webattr.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Web Attributes") skip.
    

    {&out}
        tbar-StandardBar(
            appurl + "/sys/webattr.p",
            appurl + "/sys/webattrmnt.p",
            lc-link-otherp
            ).
    ...
    {&out}
            htmlib-TableHeading(
            "System ID^left|Attribute ID^left|Attribute Value^left|HTM?|Notes"
            ) skip.
    ...
    do:
        find first b-search 
             where b-search.SystemID >= lc-search no-lock no-error.
    ...
    repeat while avail b-query:
        ...
        {&out}
            ...
            skip
            htmlib-MntTableField(html-encode(b-query.systemid),'left')
            htmlib-MntTableField(html-encode(b-query.attrid),'left')
            htmlib-MntTableField(html-encode(b-query.attrvalue),'left')
            htmlib-MntTableField(html-encode(if b-query.htmformat
                                          then 'Yes' else 'No'),'left')
            htmlib-MntTableField(html-encode(b-query.note),'left')
            tbar-StandardRow(
                rowid(b-query),
                lc-user,
                appurl + "/sys/webattrmnt.p",

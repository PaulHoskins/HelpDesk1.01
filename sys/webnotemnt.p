...
/***********************************************************************
    ...
    Program:        sys/webnotemnt.p
    
    Purpose:        Note Maintenance  	 
...
def buffer b-valid for webnote.
def buffer b-table for webnote.
...
def var lc-customercanview     as char no-undo.
...
------------------------------------------------------------------------------*/
    ...
    do:
        ...
        then run htmlib-AddErrorMessage(
                    'notecode', 
                    'You must enter the note type',
                    input-output pc-error-field,
                    input-output pc-error-msg ).
        

        if can-find(first b-valid
                    where b-valid.notecode = lc-notecode
                      and b-valid.companycode = lc-global-company
                    no-lock)
        then run htmlib-AddErrorMessage(
                    'notecode', 
                    'This note type already exists',
                    input-output pc-error-field,
                    input-output pc-error-msg ).
...
------------------------------------------------------------------------------*/
    ...
    case lc-mode:
        ...
        then assign lc-title = 'Delete'
                    ...
                    lc-submit-label = 'Delete Note Type'.
        when 'Update'
        then assign lc-title = 'Update'
                    lc-link-label = 'Cancel update'
                    lc-submit-label = 'Update Note Type'.
    end case.


    assign lc-title = lc-title + ' Note Type'
           lc-link-url = appurl + '/sys/webnote.p' + 
                                  '?search=' + lc-search + 
                                  '&firstrow=' + lc-firstrow + 
                                  '&lastrow=' + lc-lastrow + 
                                  '&navigation=refresh' +
    ...
    do:
        ...
        do:
            ...
            set-user-field("nexturl",appurl + "/sys/webnote.p").
    ...
    do:
        ...
        do:
            assign lc-notecode   = get-value("notecode")
                   lc-description  = get-value("description")
                   lc-customercanview  = get-value("customercanview")
                   .
            ...
            do:
                ...
                do:
                    create b-table.
                    assign b-table.notecode = caps(lc-notecode)
                ...
                do:
                    assign b-table.description     = lc-description
                           b-table.customercanview     = lc-customercanview = 'on'
        ...
        do:
            ...
            RUN run-web-object IN web-utilities-hdl ("sys/webnote.p").
            return.
        end.
    end.
    ...
    do:
        ...
        assign lc-notecode = b-table.notecode.

        if can-do("view,delete",lc-mode)
        or request_method <> "post"
        then assign lc-description   = b-table.description
                    lc-customercanview   = if b-table.customercanview then 'on' else ''
                    .
    ...
    {&out} htmlib-Header(lc-title) skip
           htmlib-StartForm("mainform","post", appurl + '/sys/webnotemnt.p' )
    ...
    {&out} '<TR><TD VALIGN="TOP" ALIGN="right">' 
           ...
           then htmlib-SideLabelError("Note Code")
           else htmlib-SideLabel("Note Code"))
           '</TD>' skip
           .

    if lc-mode = "ADD" then
    {&out} '<TD VALIGN="TOP" ALIGN="left">'
           htmlib-InputField("notecode",20,lc-notecode) skip
           '</TD>'.
    else
    ...
    {&out} '<TR><TD VALIGN="TOP" ALIGN="right">' 
            (if lookup("customercanview",lc-error-field,'|') > 0 
            then htmlib-SideLabelError("Customer Can View?")
            else htmlib-SideLabel("Customer Can View?"))
            '</TD>'.
    
    if not can-do("view,delete",lc-mode) then
    {&out} '<TD VALIGN="TOP" ALIGN="left">'
            htmlib-CheckBox("customercanview", if lc-customercanview = 'on'
                                        then true else false) 
            '</TD>' skip.
    else 
    {&out} htmlib-TableField(html-encode(if lc-customercanview = 'on'

...
DEFINE BUFFER b-query  FOR acs_head.
DEFINE BUFFER b-search FOR acs_head.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-Header("Maintain Account Survey") skip.
    ...
    {&out}
    tbar-Begin(
        tbar-Find(appurl + "/sys/webacs.p")
        )
    tbar-Link("add",?,appurl + "/sys/webacsmnt.p",lc-link-otherp)
    tbar-BeginOption()
    ...
    {&out}
    htmlib-TableHeading(
        "Survey Code^left|Description^left"
        ) skip.
        ...
        ELSE
            ...
            DO:
                FIND FIRST b-search 
                    ...
                    AND b-search.acs_code >= lc-search NO-LOCK NO-ERROR.
    ...
    REPEAT WHILE AVAILABLE b-query:
        ...
        ASSIGN 
            lc-link-otherp = 'search=' + lc-search +
                             ...
                             '&projectcode=' + string(b-query.acs_code).
        ...
        {&out}
            ...
            skip
            htmlib-MntTableField(html-encode(b-query.acs_code),'left')
            htmlib-MntTableField(html-encode(b-query.Descr),'left')
              tbar-BeginHidden(rowid(b-query))
                tbar-Link("view",rowid(b-query),appurl + "/sys/webacsmnt.p",lc-link-otherp)
                tbar-Link("update",rowid(b-query),appurl + "/sys/webacsmnt.p",lc-link-otherp)

...
def buffer b-query for ivClass.
def buffer b-search for ivClass.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webeqclass.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Inventory Classifications") skip.
    
    {&out}
            tbar-Begin(
                tbar-Find(appurl + "/sys/webeqclass.p")
                )
            tbar-Link("add",?,appurl + "/sys/webeqclassmnt.p",lc-link-otherp)
            tbar-BeginOption()
            tbar-Link("view",?,"off",lc-link-otherp)
            tbar-Link("update",?,"off",lc-link-otherp)
            tbar-Link("delete",?,"off",lc-link-otherp)
            tbar-Link("eqsubclass",?,"off",lc-link-otherp)
            tbar-EndOption()
            tbar-End().
    ...
    {&out}
            htmlib-TableHeading(
            "Priority^right|Classification^left|Name^left"
            ) skip.


    open query q for each b-query no-lock
        where b-query.companycode = lc-global-company
           by b-query.DisplayPriority DESC
           by b-query.ClassCode.
    ...
    do:
        find first b-search 
             where b-search.CompanyCode = lc-global-company
               and b-search.ClassCode >= lc-search no-lock no-error.
    ...
    repeat while avail b-query:
        ...
        {&out}
            ...
            skip
            htmlib-MntTableField(html-encode(string(b-query.DisplayPriority)),'right')
            htmlib-MntTableField(html-encode(b-query.ClassCode),'left')
            htmlib-MntTableField(html-encode(b-query.Name),'left')

            tbar-BeginHidden(rowid(b-query))
                tbar-Link("view",rowid(b-query),appurl + "/sys/webeqclassmnt.p",lc-link-otherp)
                tbar-Link("update",rowid(b-query),appurl + "/sys/webeqclassmnt.p",lc-link-otherp)
                tbar-Link("delete",rowid(b-query),
                          if DYNAMIC-FUNCTION('com-CanDelete':U,lc-user,"webeqclass",rowid(b-query))
                          then ( appurl + "/sys/webeqclassmnt.p") else "off",
                          lc-link-otherp)
                tbar-Link("eqsubclass",rowid(b-query),appurl + "/sys/websubclass.p",lc-link-otherp + "&init=yes&classcode=" + 
    ...
    {lib/navpanel.i "sys/webeqclass.p"}

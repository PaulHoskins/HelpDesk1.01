...
DEFINE BUFFER b-query  FOR ptp_phase.
DEFINE BUFFER b-search FOR ptp_phase.
...
PROCEDURE process-web-request :
    ...
    {&out} htmlib-Header("Maintain Project Template Phase") skip.

    {&out} htmlib-JScript-Maintenance() skip.

    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webprojphase.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Project Template Phases -<i> " + lc-ProjCode + " " + this-proj.descr + '</i>') skip.
    
    ASSIGN 
        lc-link-url = appurl + '/sys/webprojtemp.p' + 
    ...
    tbar-Link("ptask",?,"off",lc-link-otherp)
        ...
        {&out}
            ...
            htmlib-MntTableField(html-encode(b-query.Descr),'left')
              tbar-BeginHidden(rowid(b-query))
                tbar-Link("view",rowid(b-query),appurl + "/sys/webprojphasemnt.p",lc-link-otherp)
                tbar-Link("update",rowid(b-query),appurl + "/sys/webprojphasemnt.p",lc-link-otherp)
                tbar-Link("delete",rowid(b-query),
                          if DYNAMIC-FUNCTION('com-CanDelete':U,lc-user,"webprojphase",rowid(b-query))
                          then ( appurl + "/sys/webprojphasemnt.p") else "off",
                          lc-link-otherp)
                tbar-Link("recdown",rowid(b-query),
                IF lr-rows[2] = rowid(b-query) THEN "off" 
                ELSE appurl + "/sys/webprojphasemnt.p",lc-link-otherp)
                tbar-Link("recup",rowid(b-query),
                IF lr-rows[1] = rowid(b-query) THEN "off" 
                ELSE appurl + "/sys/webprojphasemnt.p",lc-link-otherp)
               
                tbar-Link("ptask",rowid(b-query),appurl + "/sys/webprojptask.p",lc-link-otherp + "&init=yes"
...
*/
    ...
    {lib/navpanel.i "sys/webprojphase.p"}

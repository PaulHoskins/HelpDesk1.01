...
/*             tbar-Link("eqsubcontract",?,"off",lc-link-otherp) */
    ...
    {&out}
            htmlib-TableHeading(
            "Type^right|Description^left|Minimum Time Bookable (Minutes)^left"
            ) skip.
    ...
    repeat while avail b-query:
        ...
        {&out}
            ...
            skip
            htmlib-MntTableField(html-encode(string(b-query.ActivityType)),'right')
            htmlib-MntTableField(html-encode(string(b-query.Description)),'left')
            htmlib-MntTableField(html-encode(string(b-query.MinTime)),'left')

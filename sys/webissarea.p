...
def var lc-grp          as char no-undo.

def buffer b-query for webissarea.
def buffer b-search for webissarea.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-Header("Maintain Issue Area") skip.

    {&out} htmlib-JScript-Maintenance() skip.

    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webissarea.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Issue Area") skip.
    
    {&out}
        tbar-StandardBar(
            appurl + "/sys/webissarea.p",
            appurl + "/sys/webissareamnt.p",
            lc-link-otherp
            ).
    ...
    {&out}
            htmlib-TableHeading(
            "Area^left|Description^left|Group^left"
            ) skip.
    ...
    do:
        find first b-search 
             where b-search.companycode = lc-global-company
               and b-search.areacode >= lc-search no-lock no-error.
    ...
    repeat while avail b-query:
        ...
        find webissagrp
            where webissagrp.CompanyCode = b-query.CompanyCode
              and webissagrp.GroupID     = b-query.GroupID
              no-lock no-error.

        if avail webIssAGrp
        then assign lc-grp = webissAGrp.Description.
        else assign lc-grp = "".
        ...
        {&out}
            ...
            skip
            htmlib-MntTableField(html-encode(b-query.areacode),'left')
            htmlib-MntTableField(html-encode(b-query.description),'left')
            htmlib-MntTableField(html-encode(lc-grp),'left')
    ...
    {lib/navpanel.i "sys/webissarea.p"}

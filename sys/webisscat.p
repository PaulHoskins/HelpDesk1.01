...
/***********************************************************************
    ...
    Program:        sys/webisscat.p
    
    Purpose:        Issue Category Maintenance       
...
def buffer b-query for webisscat.
def buffer b-search for webisscat.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-Header("Maintain Issue Category") skip.

    {&out} htmlib-JScript-Maintenance() skip.

    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webisscat.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Issue Category") skip.
    
    {&out}
        tbar-StandardBar(
            appurl + "/sys/webisscat.p",
            appurl + "/sys/webisscatmnt.p",
            lc-link-otherp
            ).
    ...
    {&out}
            htmlib-TableHeading(
            "Category^left|Description^left|Default?|Customer?"
            ) skip.
    ...
    do:
        find first b-search 
             where b-search.companycode = lc-global-company
               and b-search.catcode >= lc-search no-lock no-error.
    ...
    repeat while avail b-query:
        ...
        {&out}
            ...
            skip
            htmlib-MntTableField(html-encode(b-query.catcode),'left')
            htmlib-MntTableField(html-encode(b-query.description),'left')
            htmlib-MntTableField(if b-query.isDefault then "Yes" else "No",'left')
            htmlib-MntTableField(if b-query.Customer then "Yes" else "No",'left')
    ...
    {lib/navpanel.i "sys/webisscat.p"}

...
def buffer b-query for webissagrp.
def buffer b-search for webissagrp.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webissagrp.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Issue Area Groups") skip.
    
    {&out}
        tbar-StandardBar(
            appurl + "/sys/webissagrp.p",
            appurl + "/sys/webissagrpmnt.p",
            lc-link-otherp
            ).
    ...
    do:
        find first b-search 
             where b-search.companycode = lc-global-company
               and b-search.groupid >= lc-search no-lock no-error.
    ...
    repeat while avail b-query:
        ...
        {&out}
            ...
            skip
            htmlib-MntTableField(html-encode(b-query.groupid),'left')
    ...
    {lib/navpanel.i "sys/webissagrp.p"}

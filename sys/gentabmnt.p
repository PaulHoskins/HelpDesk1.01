...
def buffer b-valid for gentab.
def buffer b-table for gentab.
...
def var lc-gtype        as char no-undo.
def var lc-descr        as char no-undo.
def var lc-gcode        AS CHAR NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    assign lc-title = lc-title + ' General Table'
    ...
    do:
        ...
        do:
            assign lc-gtype     = get-value("gtype")
    ...
    DO:
        ...
        DO iloop = 1 TO NUM-ENTRIES(lc-global-GT-Code):
            ...
            {&out}
                '<option value="' cCode '" ' if lc-gType = cCode
